...
func _state_enter(info: Dictionary) -> void:
	host.play_animation("Wiz Idle" if host.vector.x == 0.0 else "Wiz Run")
...
func _state_physics_process(delta: float) -> void:
	...
	if input_vector != Vector2.ZERO:
			get_parent().change_state("WizRun", {"input_vector": input_vector,})
			return
	
	_decelerate_to_zero(delta)
	
	# If we just decelerated to 0, then switch to the idle animation.
	if not host.get_current_animation() in ["Wiz Idle"] and host.vector.x == 0:
		host.play_animation("Wiz Idle")
